; filename canmio.asm
;
; Routines to support the additional inputs when used in CANMIO, CANBIP or CANCDU hardware.
;
; These inputs can also be used in orginal MERG hardware
...
;
; Search the main firmwware source file for "MIO" to find references to this source module
;
; Rev 1.0 01/02/14 PNB  original coded
; Rev 1.1 08/08/15 PNB  updated for canace8mio
; Rev 1.2 18/02/17 PNB  Make canace8mio code 25k80 compatible (clear EEADRH before EEPROM operation)        
;
...
dbloops     equ .100              ; Debounce count - number of times round main loop inputs must be stable before event sent
...
inpsod
        movf    SoDing,w            ; Check if SOD required
        bz      sodend              ; Skip this routine if not
...
getonof	movlw	Modstat             ;get setup status
		movwf	EEADR
		call	eeread
		bcf		Mode,0
...
mionewnn
		...
		btfsc	PORTA,MIO_ONONLY ;is it ON only?
        bra     onoff
...
mioSLiM                         
        ...
        btfsc   STATUS,Z        ; zero flag set if no output selected
        bra     go_on1          ;   then not in learn mode
        ...
        bz      mlearnp         ; yes
        movf    switches,w
        sublw   MIOLRNINV       ; SLiM learn with inverted polarity
        bz      mlearni
        movf    switches,w
        sublw   MIOLRNCNV       ; SLiM learn with inverted polarity
        bz      mlearni
        movf    switches,w
        sublw   MIOLRNDIR       ; SLiM learn 
        bnz     chkunl

mlearnp bsf     switches,MIO_POL    ; Normal polarity
        bra     mlearnc

mlearni bcf     switches,MIO_POL    ; Inverted polarity

mlearnc call    enmatch         ; Uses Temp,returns zero if found
        sublw   0
        bnz     newevm
...
getopm  movlw	MIO_OP_MASK		;get DIP switch setting for output
		andwf	PORTB,W
		movwf	Temp
		movwf	Temp1
		rrncf	Temp1,F
        ...
        movlw   B'00001111'
        andwf   Temp,F
        bz      opdun           ; Abort if output number selected as zero
        decf    Temp            ; So that 1 to 8 on switches gives 0-7
#ifdef  CANCDU
        movff   Temp,Shift      ; CANACC4 firmware uses a different memory lcoation
#endif
opdun   return                 ; Continue routine with common code in getop
